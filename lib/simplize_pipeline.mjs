...
function extractKpis(item) {
  const rows = [];
  for (const [k, v] of Object.entries(item)) {
    if (k === 'periodDate' || k === 'periodDateName' || k === 'ticker') continue;
    if (typeof v !== 'number' || Number.isNaN(v)) continue;
    rows.push({ metric: k, value: v, statement: 'kpi' });
  }
  return rows;
}
...
export function normalizeBlock(block) {
  ...
  // Clean, named KPIs (used for screener presets): agg-compare/overview
  const kpiItems = block?.data?.aggCompareOverview?.body?.data?.items;
  if (Array.isArray(kpiItems)) {
    for (const item of kpiItems) {
      const periodDate = item.periodDate ?? null;
      const periodDateName = item.periodDateName ?? null;
      const kpis = extractKpis(item);
      for (const m of kpis) {
        out.push({
          ticker,
          period,
